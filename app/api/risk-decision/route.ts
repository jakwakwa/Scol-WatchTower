...
export async function POST(request: NextRequest) {
    try {
        ...
        const workflowResult = await db.select().from(workflows).where(eq(workflows.id, workflowId));
        ...
        if (workflow.status !== 'awaiting_human' && workflow.status !== 'pending') {
            console.warn(`[RiskDecision] Workflow ${workflowId} is in unexpected state: ${workflow.status}`);
...
export async function GET(request: NextRequest) {
    try {
        ...
        const pendingWorkflows = await db.select().from(workflows).where(eq(workflows.status, 'awaiting_human'));
