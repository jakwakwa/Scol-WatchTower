...
export interface LogEventParams {
    workflowId: number;
    eventType: 'stage_change' | 'agent_dispatch' | 'agent_callback' | 'human_override' | 'timeout' | 'error';
    payload: object;
 ...
 */
export async function createWorkflowNotification(params: CreateNotificationParams): Promise<void> {
    console.log(`[NotificationEvents] Creating notification: ${params.title} - ${params.message}`);
...
export async function logWorkflowEvent(params: LogEventParams): Promise<void> {
    console.log(`[NotificationEvents] Logging event: ${params.eventType} for workflow ${params.workflowId}`);
