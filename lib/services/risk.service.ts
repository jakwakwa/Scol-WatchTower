...
export async function analyzeRisk(leadId: number): Promise<RiskResult> {
    ...
    if (!aiServiceUrl) {
        throw new Error('[RiskService] WEBHOOK_ZAP_AI_RISK_ANALYSIS not configured');
    }
    ...
    if (!response.ok) {
        throw new Error(`AI risk service failed: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();

    if (result.riskScore === undefined) {
        throw new Error(`[RiskService] Invalid response: ${JSON.stringify(result)}`);
    }
