...
export const DirectorInfoSchema = z.object({
    ...
    /** Director role */
    role: z.enum(['DIRECTOR', 'MANAGING_DIRECTOR', 'CEO', 'CFO', 'SHAREHOLDER']).default('DIRECTOR'),
    /** Email address */
...
export const FacilityApplicationSchema = z.object({
    ...
    registrationNumber: z
        .string()
        .regex(/^\d{4}\/\d{6}\/\d{2}$/, 'Registration format: YYYY/NNNNNN/07 (e.g., 2020/123456/07)')
        .optional(),
...
export const ITCCheckResultSchema = z.object({
    ...
    recommendation: z.enum(['AUTO_APPROVE', 'MANUAL_REVIEW', 'AUTO_DECLINE', 'ENHANCED_DUE_DILIGENCE']),
