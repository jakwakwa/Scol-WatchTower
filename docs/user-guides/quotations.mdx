...
Quotes are generated automatically by Gemini AI when an applicant reaches Stage 2 of the onboarding workflow. The quote is stored as a draft and requires staff approval before the client can respond. The client can then **approve or decline** the quote, and the workflow continues or terminates accordingly.
