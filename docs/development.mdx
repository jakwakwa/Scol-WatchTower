...
Follow these steps to install and run on your operating system.

<Steps>
<Step title="Install the Control Tower Onboard CLI">
...
By default, Control Tower Onboarduses port 3000. You can customize the port Control Tower Onboardruns on by using the `--port` flag. For example, to run Control Tower Onboardon port 3333, use this command:

```bash
mint dev --port 3333
```

If you attempt to run Control Tower Onboard on a port that's already in use, it will use the next available port:

```md
Port 3000 is already in use. Trying 3001 instead.
```

## Control Tower Onboard versions
