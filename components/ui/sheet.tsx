...
function SheetOverlay({
	...
	return (
		<SheetPrimitive.Overlay
			...
			className={cn(
				"data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 bg-black/50 duration-100 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs fixed inset-0 z-50",
				className
			)}
			{...props}
		/>
			...
			<SheetPrimitive.Content
				...
				{showCloseButton && (
					<SheetPrimitive.Close data-slot="sheet-close" asChild>
						<Button variant="ghost" className="absolute top-2 right-2 p-0" size="icon-sm">
							<RiCloseCircleFill className="text-white m-4 w-8 h-8 animate-pulse" />
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn("gap-1.5 py-6 px-0 flex flex-col", className)}
			{...props}
		/>
	);
}
